"""
knn_model_test.py — Test KNN-based prediction vs bucket model.

Instead of binning cells into context buckets and averaging, find the K nearest
neighbors across all training data using continuous features:
  - terrain_code (categorical, must match)
  - ocean_distance (continuous)
  - settlement_distance (continuous)
  - forest_neighbor_count (continuous)
  - settlement_neighbor_count (continuous)

KNN advantages over bucket model:
  - Continuous distance metric (no arbitrary bin boundaries)
  - Adaptive to local data density
  - Captures feature interactions naturally

python -m scripts.knn_model_test
"""
...
def extract_features(ig, y, x, odm, sdm):
    """Extract continuous features for a cell."""
    code = ig[y][x]
    od = odm[y][x] if odm else 99
    sd = sdm[y][x] if sdm else 99
    fn = cn(ig, y, x, 4)   # forest neighbors
    sn = cn(ig, y, x, 1)   # settlement neighbors
    on = cn(ig, y, x, 10)  # ocean neighbors
    return (code, od, sd, fn, sn, on)


def knn_predict(query_feat, train_data_by_code, K):
    """Find K nearest neighbors by feature distance, return averaged distribution."""
    code = query_feat[0]
    if code not in train_data_by_code or not train_data_by_code[code]:
        return [1.0/N]*N

    candidates = train_data_by_code[code]

    # Feature weights (tunable)
    # ocean_dist, sett_dist are most important (range 0-40)
    # neighbor counts are secondary (range 0-24)
    w_od = 1.0   # ocean distance
    w_sd = 1.0   # settlement distance
    w_fn = 0.3   # forest neighbors (less important, already captured by od/sd)
    w_sn = 0.5   # settlement neighbors
    w_on = 0.3   # ocean neighbors

    _, qod, qsd, qfn, qsn, qon = query_feat

    # Compute distances
    scored = []
    for feat, gt_dist in candidates:
    ...
    if neighbors[0][0] < 1e-12:
        # Exact match: just average exact matches
        exact = [gt for d, gt in neighbors if d < 1e-12]
        avg = [sum(g[i] for g in exact)/len(exact) for i in range(N)]
    else:
        total_w = 0
        avg = [0.0]*N
        for d, gt_dist in neighbors:
            w = 1.0 / (d + 0.1)  # add small epsilon to avoid div by zero
            for i in range(N):
                avg[i] += w * gt_dist[i]
            total_w += w
...
def run_knn_model(ig, gt, train_data_by_code, odm, sdm, ao, K, use_shift=True, bucket_cm=None):
    """Run KNN-based prediction."""
    # Build KNN prior for each cell
    knn_priors = {}
    for y in range(40):
        for x in range(40):
            code = ig[y][x]
            if code in STATIC_CODES: continue
            feat = extract_features(ig, y, x, odm, sdm)
            knn_priors[(y, x)] = knn_predict(feat, train_data_by_code, K)

    # Optional: apply global shift from observations
    if use_shift and bucket_cm:
        rc = defaultdict(list)
    ...
    for y in range(40):
        ...
        for x in range(40):
            ...
            if code in STATIC_CODES:
                ...
                prior = knn_priors.get((y, x), [1.0/N]*N)[:]
...
def run_bucket_model(ig, gt, cm, odm, sdm, ao):
    """Standard bucket model (PROD)."""
    rc = defaultdict(list)
    for yx, vals in ao.items():
        c = ig[yx[0]][yx[1]]
        if c not in STATIC_CODES:
...
def main():
    ...
    K_VALUES = [50, 200, 500]
    strategies = ["PROD"] + [f"KNN_{k}" for k in K_VALUES] + [f"KNN_{K_VALUES[1]}_NS"]

    SEEDS = [42]  # Single seed to save time (KNN is slower)

    print(f"\nKNN Model Test: {len(files)} files, {len(rounds)} rounds")
    print(f"KNN: continuous features, inverse-distance weighted average")
    print(f"Features: ocean_dist, sett_dist, forest_n, sett_n, ocean_n (grouped by terrain code)")
    print("=" * 90)
    hdr = f"  {'Round':<10}"
    for s in strategies: hdr += f" {s:>12}"
    print(hdr)
    print(f"  {'-'*10}" + f" {'-'*12}" * len(strategies))

    totals = {s: [] for s in strategies}

    for trid in round_ids:
        # Build training data
        ca_bucket = defaultdict(list)
        train_data_by_code = defaultdict(list)
        ...
        for f in files:
            ...
            for y in range(40):
                for x in range(40):
                    c = ig[y][x]
                    if c not in STATIC_CODES:
                        ca_bucket[ctx_fn(ig, y, x, odm, sdm)].append(gt[y][x])
                        feat = extract_features(ig, y, x, odm, sdm)
                        train_data_by_code[c].append((feat, gt[y][x]))

        cm_bucket = {c: [sum(s[i] for s in ss)/len(ss) for i in range(N)] for c, ss in ca_bucket.items()}
        ...
        for fname in rounds[trid]:
            ...
            for seed in SEEDS:
                ...
                # PROD (bucket model)
                sc = run_bucket_model(ig, gt, cm_bucket, odm, sdm, ao)
                round_scores["PROD"].append(sc)

                # KNN models
                for k in K_VALUES:
                    sc = run_knn_model(ig, gt, train_data_by_code, odm, sdm, ao, K=k,
                                       use_shift=True, bucket_cm=cm_bucket)
                    round_scores[f"KNN_{k}"].append(sc)

                # KNN without shift
                k = K_VALUES[1]
                sc = run_knn_model(ig, gt, train_data_by_code, odm, sdm, ao, K=k,
                                   use_shift=False)
                round_scores[f"KNN_{k}_NS"].append(sc)
        ...
        for s in strategies:
            m = "*" if s == best else " "
            line += f" {avgs[s]:11.1f}{m}"
        print(line)

    print(f"\n{'='*90}")
    prod_avg = sum(totals["PROD"]) / len(totals["PROD"])
    print(f"\n  {'Strategy':<14} {'Avg':>7} {'Delta':>7} {'Min':>7} {'Std':>7}")
    print(f"  {'-'*14} {'-'*7} {'-'*7} {'-'*7} {'-'*7}")
    for s in strategies:
        sc = totals[s]
        avg = sum(sc)/len(sc)
        ...
        print(f"  {s:<14} {avg:7.2f} {delta:+7.2f} {mn:7.1f} {std:7.1f}{marker}")

    # Data size info
    for code in sorted(train_data_by_code):
        print(f"\n  Code {code}: {len(train_data_by_code[code])} training cells")
