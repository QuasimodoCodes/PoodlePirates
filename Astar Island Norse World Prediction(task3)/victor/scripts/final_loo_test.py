"""
final_loo_test.py — Full per-round LOO with current production config.

Tests OLD baseline vs FINAL (Forest features + NHS20).
Shows per-round scores and overall summary.

python -m scripts.final_loo_test
"""
...
def count_neighbors(ig, y, x, tc, r=2):
    c = 0
...
def ctx_old(ig, y, x, odm, sdm):
    """OLD baseline: 19 buckets, binary ocean flag."""
    code = ig[y][x]
    sn = count_neighbors(ig, y, x, 1)
    on = count_neighbors(ig, y, x, 10)
    sb = "sh" if sn >= 3 else ("sl" if sn >= 1 else "sn")
    ob = "oc" if on >= 1 else "in"
    return (code, sb, ob)

def ctx_final(ig, y, x, odm, sdm):
    """FINAL: dist_ocean + dist_sett for Plains AND Forest."""
    code = ig[y][x]
    sn = count_neighbors(ig, y, x, 1)
    sb = "sh" if sn >= 3 else ("sl" if sn >= 1 else "sn")
    if code in (4, 11) and odm and sdm:
        return (code, bin_sd(sdm[y][x]), bin_od(odm[y][x]))
    elif code in (4, 11) and odm:
...
def run_model(ig, gt, cm, ctx_fn, odm, sdm, TEMP, N_HIST, N_HIST_SURP, SURP_THRESH):
    ao = get_obs(ig, gt)
    rc = defaultdict(list)
...
def main():
    ...
    # (label, ctx_fn, TEMP, N_HIST, N_HIST_SURP, SURP_THRESH)
    strategies = [
        ("OLD",   ctx_old,   1.10, 50,  5, 0.30),
        ("FINAL", ctx_final, 1.10, 50, 20, 0.30),
    ]
    labels = [s[0] for s in strategies]

    print(f"\nFinal LOO Test: {len(files)} files, {len(rounds)} rounds")
    print(f"OLD:   19 buckets, NHS=5")
    print(f"FINAL: Forest+Plains spatial features, NHS=20")
    print("=" * 80)
    print(f"  {'Round':<10} {'OLD':>8} {'FINAL':>8} {'Delta':>8}  Note")
    print(f"  {'-'*10} {'-'*8} {'-'*8} {'-'*8}  {'-'*20}")
    ...
    for trid in round_ids:
        cms = {}
        for label, ctx_fn, *_ in strategies:
            ca = defaultdict(list)
            for f in files:
        ...
        for fname in rounds[trid]:
            ...
            sdm = sdm_c.get(fname)
            rng = random.getstate()
            for label, ctx_fn, TEMP, NH, NHS, ST in strategies:
                random.setstate(rng)
                sc = run_model(ig, gt, cms[label], ctx_fn, odm, sdm, TEMP, NH, NHS, ST)
                round_scores[label].append(sc)

        old_avg = sum(round_scores["OLD"]) / len(round_scores["OLD"])
        fin_avg = sum(round_scores["FINAL"]) / len(round_scores["FINAL"])
        delta = fin_avg - old_avg
        totals["OLD"].extend(round_scores["OLD"])
        totals["FINAL"].extend(round_scores["FINAL"])

        note = ""
        if delta > 3: note = "BIG WIN"
        elif delta > 1: note = "win"
        elif delta < -1: note = "REGRESSION"
        elif delta < 0: note = "slight loss"

        short = trid[:8]
        print(f"  {short:<10} {old_avg:8.1f} {fin_avg:8.1f} {delta:+8.1f}  {note}")

    # Summary
    print(f"\n{'='*80}")
    old_all = totals["OLD"]
    fin_all = totals["FINAL"]
    old_avg = sum(old_all) / len(old_all)
    fin_avg = sum(fin_all) / len(fin_all)
    old_std = (sum((s - old_avg)**2 for s in old_all) / len(old_all)) ** 0.5
    fin_std = (sum((s - fin_avg)**2 for s in fin_all) / len(fin_all)) ** 0.5

    print(f"\n  {'Config':<10} {'Avg':>7} {'Delta':>7} {'Min':>7} {'Max':>7} {'Std':>7}")
    print(f"  {'-'*10} {'-'*7} {'-'*7} {'-'*7} {'-'*7} {'-'*7}")
    print(f"  {'OLD':<10} {old_avg:7.2f} {0:+7.2f} {min(old_all):7.1f} {max(old_all):7.1f} {old_std:7.1f}")
    print(f"  {'FINAL':<10} {fin_avg:7.2f} {fin_avg-old_avg:+7.2f} {min(fin_all):7.1f} {max(fin_all):7.1f} {fin_std:7.1f}")

    wins = sum(1 for o, f in zip(old_all, fin_all) if f > o)
    losses = sum(1 for o, f in zip(old_all, fin_all) if f < o)
    ties = len(old_all) - wins - losses
    print(f"\n  Per-seed: {wins} wins, {losses} losses, {ties} ties out of {len(old_all)}")
    print(f"\n  Done.")
