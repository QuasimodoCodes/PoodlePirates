"""
temp_floor_sweep.py — Sweep temperature and floor values with new pipeline.

Diagnostics showed:
  - Temperature hurts (+0.00171 KL)
  - 100% cells floored at 0.001
  - Under-predicting confidence by 3-4%

python -m scripts.temp_floor_sweep
"""
...
def run_model(ig, gt, cm_hist, temp, floor):
    ao = get_obs(ig, gt)
    rc = defaultdict(list)
...
def main():
    ...
    print(f"Temp+floor sweep: {len(files)} files, {len(rounds)} rounds")
    print("=" * 110)

    configs = [
        # (label, temp, floor)
        ("A.t1.10_f.001", 1.10, 0.001),   # current
        ("B.t1.05_f.001", 1.05, 0.001),
        ("C.t1.02_f.001", 1.02, 0.001),
        ("D.t1.00_f.001", 1.00, 0.001),
        ("E.t0.98_f.001", 0.98, 0.001),
        ("F.t0.95_f.001", 0.95, 0.001),
        ("G.t1.00_f.0005", 1.00, 0.0005),
        ("H.t1.00_f.0001", 1.00, 0.0001),
        ("I.t1.02_f.0005", 1.02, 0.0005),
        ("J.t0.98_f.0005", 0.98, 0.0005),
    ]
    ...
    for test_rid in round_ids:
        ...
        for fname in rounds[test_rid]:
            ...
            if not gt or not ig: continue
            for label, temp, floor in configs:
                sc = run_model(ig, gt, cm_hist, temp, floor)
