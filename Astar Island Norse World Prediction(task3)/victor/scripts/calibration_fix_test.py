"""
calibration_fix_test.py - Test fixes for high-bin underprediction.

Diagnostics showed we underpredict in the 60-100% bins (predicting 70% when GT is 78%).
This is likely from temperature=1.10 softening confident predictions too much.

Test strategies:
  A. Current model (temp=1.10)
  B. Lower temps: 1.05, 1.03, 1.02, 1.00
  C. Post-hoc power recalibration: p_new = p^gamma / sum(p^gamma)
     gamma < 1 sharpens (pushes toward extremes), gamma > 1 softens
  D. Combined: lower temp + power recalibration

python -m scripts.calibration_fix_test
"""
...
N = 6; FS = 1e-5; FD = 0.005; ALPHA = 0.05; N_MC = 8
...
def power_recal(d, gamma):
    """Power recalibration: p^gamma / sum(p^gamma). gamma<1 sharpens."""
...
def run_model(ig, gt, cm, temp=1.10, gamma=None):
    """Run full model with configurable temperature and optional power recal."""
    ao = get_obs(ig, gt)
    rc = defaultdict(list)
...
def main():
    ...
    strategies = [
        # (label, temp, gamma)
        ("A. temp=1.10 (current)", 1.10, None),
        ("B. temp=1.05",           1.05, None),
        ("C. temp=1.03",           1.03, None),
        ("D. temp=1.02",           1.02, None),
        ("E. temp=1.00 (no temp)", 1.00, None),
        ("F. temp=0.98 (sharpen)", 0.98, None),
        ("G. t=1.10 + g=0.95",    1.10, 0.95),
        ("H. t=1.10 + g=0.90",    1.10, 0.90),
        ("I. t=1.10 + g=0.85",    1.10, 0.85),
        ("J. t=1.05 + g=0.95",    1.05, 0.95),
        ("K. t=1.05 + g=0.90",    1.05, 0.90),
        ("L. t=1.03 + g=0.95",    1.03, 0.95),
    ]

    print(f"Calibration fix test: {len(files)} files, {len(rounds)} rounds, {N_MC} MC")
    print(f"Testing temperature and power recalibration fixes")
    print()

    # Header
    labels = [s[0].split(".")[0].strip() for s in strategies]
