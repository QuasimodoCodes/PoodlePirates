"""
hyperparam_sweep.py — Multi-seed sweep of TEMP and N_HIST_SURP.

Tests combinations across 5 random seeds to filter noise from signal.
...
def run_model(ig, gt, cm, odm, sdm, TEMP, N_HIST, N_HIST_SURP, SURP_THRESH, ALPHA):
    ao = get_obs(ig, gt)
    rc = defaultdict(list)
...
def main():
    ...
    # (label, TEMP, N_HIST, N_HIST_SURP, SURP_THRESH, ALPHA)
    configs = [
        ("PROD",             1.10, 50,  5, 0.30, 0.05),
        ("T1.15",            1.15, 50,  5, 0.30, 0.05),
        ("T1.20",            1.20, 50,  5, 0.30, 0.05),
        ("T1.25",            1.25, 50,  5, 0.30, 0.05),
        ("NHS10",            1.10, 50, 10, 0.30, 0.05),
        ("NHS15",            1.10, 50, 15, 0.30, 0.05),
        ("NHS20",            1.10, 50, 20, 0.30, 0.05),
        ("NHS10+ST.35",      1.10, 50, 10, 0.35, 0.05),
        ("T1.15+NHS10",      1.15, 50, 10, 0.30, 0.05),
        ("T1.20+NHS10",      1.20, 50, 10, 0.30, 0.05),
        ("T1.15+NHS10+ST35", 1.15, 50, 10, 0.35, 0.05),
    ]
    labels = [c[0] for c in configs]

    SEEDS = [42, 123, 456, 789, 1337]

    print(f"\nHyperparam sweep: {len(files)} files, {len(rounds)} rounds, {len(SEEDS)} seeds")
    ...
    for si, seed_val in enumerate(SEEDS):
        print(f"  Seed {seed_val} ({si+1}/{len(SEEDS)})...", flush=True)
        random.seed(seed_val)
        seed_totals = {l: [] for l in labels}

        for trid in round_ids:
            # Build conditional matrix (same for all configs)
            ca = defaultdict(list)
            for f in files:
            ...
            for fname in rounds[trid]:
                ...
                sdm = sdm_c.get(fname)
                rng = random.getstate()
                for label, TEMP, NH, NHS, ST, ALP in configs:
                    random.setstate(rng)
                    sc = run_model(ig, gt, cm, odm, sdm, TEMP, NH, NHS, ST, ALP)
                    seed_totals[label].append(sc)

        for l in labels:
            avg = sum(seed_totals[l]) / len(seed_totals[l])
            all_results[l].append(avg)

    # Print results
    prod_avgs = all_results["PROD"]
    prod_mean = sum(prod_avgs) / len(prod_avgs)

    print(f"\n{'='*120}")
    print(f"\n  {'Config':<22}", end="")
    for s in SEEDS:
        print(f"  s={s:<5}", end="")
    print(f"  {'Mean':>7} {'Delta':>7} {'Wins':>5}")
    print(f"  {'-'*22}", end="")
    for _ in SEEDS:
        print(f"  {'-'*7}", end="")
    print(f"  {'-'*7} {'-'*7} {'-'*5}")
    ...
    for label, *_ in configs:
        avgs = all_results[label]
        mean = sum(avgs) / len(avgs)
        delta = mean - prod_mean
        wins = sum(1 for a, p in zip(avgs, prod_avgs) if a > p)
        print(f"  {label:<22}", end="")
        for a in avgs:
            print(f"  {a:7.2f}", end="")
        marker = " <<<" if wins >= 4 else ""
        print(f"  {mean:7.2f} {delta:+7.2f} {wins:>3}/5{marker}")
