"""
all_terrain_features_test.py — Test spatial features for ALL terrain types.

Currently: Plains + Forest get (dist_sett, dist_ocean) spatial features.
Test adding spatial features to Settlement and Port too.

python -m scripts.all_terrain_features_test
"""
...
def make_ctx_fn(spatial_codes):
    """Create a context function that uses spatial features for given terrain codes."""
    def ctx_fn(ig, y, x, odm, sdm):
        code = ig[y][x]
        sn = cn(ig, y, x, 1)
        sb = "sh" if sn >= 3 else ("sl" if sn >= 1 else "sn")
        if code in spatial_codes and odm and sdm:
...
def main():
    ...
    # Strategies: which terrain codes get spatial features
    # Current production: {4, 11} (Forest + Plains)
    strategies = [
        ("PF (prod)",      {4, 11}),           # current production
        ("PFS",            {1, 4, 11}),         # + Settlement
        ("PFP",            {2, 4, 11}),         # + Port
        ("PFSP",           {1, 2, 4, 11}),      # + Settlement + Port
        ("P only",         {11}),               # Plains only (regression check)
        ("F only",         {4}),                # Forest only (regression check)
        ("All dynamic",    {0, 1, 2, 3, 4, 11}),# all dynamic codes
    ]

    labels = [s[0] for s in strategies]
    ctx_fns = {label: make_ctx_fn(codes) for label, codes in strategies}

    print(f"\nAll-Terrain Spatial Features Test: {len(files)} files, {len(rounds)} rounds")
    print(f"Which terrain types benefit from (dist_sett, dist_ocean) spatial bucketing?")
    print(f"Cell counts: Plains=107k, Forest=37k, Settlement=4.8k, Port=194")
    print("=" * 100)
    ...
    for trid in round_ids:
            ...
            for f in files:
                ...
                for y in range(40):
                    for x in range(40):
                        c = ig[y][x]
                        if c not in STATIC_CODES:
                            ca[ctx_fns[label](ig, y, x, odm, sdm)].append(gt[y][x])
            cms[label] = {c: [sum(s[i] for s in ss)/len(ss) for i in range(N)] for c, ss in ca.items()}
        ...
        for fname in rounds[trid]:
            ...
            for label in labels:
                random.setstate(rng)
                sc = run_model(ig, gt, cms[label], ctx_fns[label], odm, sdm)
                rscores[label].append(sc)

        short = trid[:8]
        avgs = {l: sum(rscores[l])/len(rscores[l]) for l in labels}
        best = max(avgs, key=avgs.get)
        print(f"  {short:<10}", end="")
        for l in labels:
            totals[l].extend(rscores[l])
            m = "*" if l == best else " "
            print(f" {avgs[l]:10.1f}{m}", end="")
        print()

    # Summary
    print(f"\n{'='*100}")
    prod_avg = sum(totals["PF (prod)"]) / len(totals["PF (prod)"])
    print(f"\n  {'Config':<14} {'Codes':<20} {'Avg':>7} {'Delta':>7} {'Min':>7} {'Std':>7} {'Buckets':>8}")
    print(f"  {'-'*14} {'-'*20} {'-'*7} {'-'*7} {'-'*7} {'-'*7} {'-'*8}")
    for label, codes in strategies:
        scores = totals[label]
        avg = sum(scores) / len(scores)
        mn = min(scores)
        std = (sum((s - avg)**2 for s in scores) / len(scores)) ** 0.5
        delta = avg - prod_avg
        code_str = ",".join(str(c) for c in sorted(codes))
        # Count approximate buckets
        n_spatial = len(codes) * 16  # 4 od bins * 4 sd bins
        n_other = (6 - len(codes)) * 6  # 3 sett bins * 2 ocean bins
        marker = " <<<" if delta > 0.3 else ""
        print(f"  {label:<14} {code_str:<20} {avg:7.2f} {delta:+7.2f} {mn:7.1f} {std:7.1f} {n_spatial+n_other:>8}{marker}")
