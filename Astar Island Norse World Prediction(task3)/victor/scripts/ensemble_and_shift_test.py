"""
ensemble_and_shift_test.py — Test ensembling and improved shift estimation.

Ceiling analysis showed:
- Oracle CM only +0.20 → CM is near-optimal
- Global shift worth +1.55 → improving shift is high leverage
- More observations hurt → noise is the enemy

Strategies:
  PROD:        Current production
  ENS_TEMP:    Ensemble of 3 temperatures (1.05, 1.10, 1.15), averaged
  ENS_NHIST:   Ensemble of 3 N_HIST values (30, 50, 80), averaged
  ENS_BOTH:    Ensemble of 9 (3 temps x 3 nhist), averaged
  SHIFT_CLASS: Per-terrain-code shift instead of global shift
  SHIFT_SOFT:  Softer shift: cube root instead of sqrt
  SHIFT_ADAPT: Adaptive shift strength based on observation count
  N_HIST_40:   N_HIST=40 instead of 50 (trust observations slightly more)

python -m scripts.ensemble_and_shift_test
"""

import os, sys, json, math, random
from collections import defaultdict

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import config
from src.model.initial_analyzer import CODE_TO_CLASS, STATIC_CODES
from src.observation.adaptive_planner import SPREAD_ANCHORS, TILE_W, TILE_H, _entropy, _covered_cells

N = 6; FS = 1e-5; FD = 0.001; ALPHA = 0.05
N_HIST_SURP = 20; SURP_THRESH = 0.30

ALL_A = [(ax, ay) for ay in range(40 - TILE_H + 1) for ax in range(40 - TILE_W + 1)]
AC = {a: set(_covered_cells(*a)) for a in ALL_A}


def bfs_dist(ig, tc):
    H, W = 40, 40
    dist = [[99]*W for _ in range(H)]; q = []
    for y in range(H):
        for x in range(W):
            if ig[y][x] == tc: dist[y][x] = 0; q.append((y, x))
    head = 0
    while head < len(q):
        cy, cx = q[head]; head += 1
        for dy, dx in [(-1,0),(1,0),(0,-1),(0,1)]:
            ny, nx = cy+dy, cx+dx
            if 0 <= ny < H and 0 <= nx < W and dist[ny][nx] > dist[cy][cx]+1:
                dist[ny][nx] = dist[cy][cx]+1; q.append((ny, nx))
    return dist

def bin_od(d):
    if d <= 1: return "od0"
    if d <= 4: return "od1"
    if d <= 10: return "od2"
    return "od3"
def bin_sd(d):
    if d <= 2: return "sd_close"
    if d <= 5: return "sd_mid"
    if d <= 10: return "sd_far"
    return "sd_void"
def kl_div(p, q):
    return sum(pi * math.log(pi / max(qi, 1e-12)) for pi, qi in zip(p, q) if pi > 1e-12)
def score_t(pred, gt):
    wkl = te = 0.0
    for y in range(40):
        for x in range(40):
            e = _entropy(gt[y][x]); wkl += e * kl_div(gt[y][x], pred[y][x]); te += e
    return max(0, min(100, 100 * math.exp(-3 * (wkl / te)))) if te > 1e-12 else 100
def so(d):
    r = random.random(); cs = 0.0
    for i, p in enumerate(d):
        cs += p
        if r < cs: return i
    return 5
def cn(ig, y, x, tc, r=2):
    c = 0
    for dy in range(-r, r+1):
        for dx in range(-r, r+1):
            if dy == 0 and dx == 0: continue
            ny, nx = y+dy, x+dx
            if 0 <= ny < 40 and 0 <= nx < 40 and ig[ny][nx] == tc: c += 1
    return c
def ts(d, t):
    if t == 1.0: s = sum(d); return [v/s for v in d]
    ld = [math.log(max(p, 1e-12))/t for p in d]; mx = max(ld)
    ed = [math.exp(v - mx) for v in ld]; s = sum(ed)
    return [v/s for v in ed]
def surprise(ol, h):
    nr = len(ol)
    if nr < 3: return 0.0
    rf = [0.0]*N
    for v in ol: rf[v] += 1.0/nr
    kf = sum(rf[i]*math.log(max(rf[i],1e-12)/max(h[i],1e-12)) for i in range(N) if rf[i] > 1e-12)
    kr = sum(h[i]*math.log(max(h[i],1e-12)/max(rf[i],1e-12)) for i in range(N) if h[i] > 1e-12)
    return (kf + kr) / 2

def compute_shift(rc, cm):
    rt = [0.0]*N; no = 0
    for ct, ol in rc.items():
        for cls in ol: rt[cls] += 1; no += 1
    if no == 0: return [1.0]*N
    rf = [c/no for c in rt]
    ht = [0.0]*N; nb = 0
    for ct, d in cm.items():
        for i in range(N): ht[i] += d[i]
        nb += 1
    if nb == 0: return [1.0]*N
    hf = [h/nb for h in ht]
    return [math.sqrt(rf[i]/hf[i]) if hf[i] > 1e-8 else 1.0 for i in range(N)]

def compute_shift_soft(rc, cm):
    """Softer shift: cube root instead of sqrt."""
    rt = [0.0]*N; no = 0
    for ct, ol in rc.items():
        for cls in ol: rt[cls] += 1; no += 1
    if no == 0: return [1.0]*N
    rf = [c/no for c in rt]
    ht = [0.0]*N; nb = 0
    for ct, d in cm.items():
        for i in range(N): ht[i] += d[i]
        nb += 1
    if nb == 0: return [1.0]*N
    hf = [h/nb for h in ht]
    return [(rf[i]/hf[i])**(1/3) if hf[i] > 1e-8 else 1.0 for i in range(N)]

def compute_shift_per_terrain(rc, cm):
    """Per-terrain-code shift instead of global."""
    shifts = {}
    # Group by terrain code (first element of context tuple)
    by_code = defaultdict(lambda: (defaultdict(list), {}))
    for ct, ol in rc.items():
        code = ct[0]
        by_code[code][0][ct] = ol
    for ct, d in cm.items():
        code = ct[0]
        by_code[code][1][ct] = d

    for code, (rc_c, cm_c) in by_code.items():
        rt = [0.0]*N; no = 0
        for ct, ol in rc_c.items():
            for cls in ol: rt[cls] += 1; no += 1
        if no == 0:
            shifts[code] = [1.0]*N
            continue
        rf = [c/no for c in rt]
        ht = [0.0]*N; nb = 0
        for ct, d in cm_c.items():
            for i in range(N): ht[i] += d[i]
            nb += 1
        if nb == 0:
            shifts[code] = [1.0]*N
            continue
        hf = [h/nb for h in ht]
        shifts[code] = [math.sqrt(rf[i]/hf[i]) if hf[i] > 1e-8 else 1.0 for i in range(N)]
    return shifts

def compute_shift_adaptive(rc, cm):
    """Adaptive: blend global shift toward 1.0 based on observation count."""
    rt = [0.0]*N; no = 0
    for ct, ol in rc.items():
        for cls in ol: rt[cls] += 1; no += 1
    if no == 0: return [1.0]*N
    rf = [c/no for c in rt]
    ht = [0.0]*N; nb = 0
    for ct, d in cm.items():
        for i in range(N): ht[i] += d[i]
        nb += 1
    if nb == 0: return [1.0]*N
    hf = [h/nb for h in ht]
    raw = [math.sqrt(rf[i]/hf[i]) if hf[i] > 1e-8 else 1.0 for i in range(N)]
    # Dampen shift when few observations
    confidence = min(1.0, no / 500)  # full confidence at 500+ obs
    return [1.0 + confidence * (r - 1.0) for r in raw]

def apply_shift(d, R):
    s = [max(d[i]*R[i], 1e-12) for i in range(N)]
    t = sum(s); return [v/t for v in s]

def ctx_fn(ig, y, x, odm, sdm):
    code = ig[y][x]; sn = cn(ig, y, x, 1)
    sb = "sh" if sn >= 3 else ("sl" if sn >= 1 else "sn")
    if code in (1, 4, 11) and odm and sdm:
        return (code, bin_sd(sdm[y][x]), bin_od(odm[y][x]))
    else:
        on = cn(ig, y, x, 10)
        return (code, sb, "oc" if on >= 1 else "in")

def sel_sett(ig, n=5):
    ss = {(y,x) for y in range(40) for x in range(40) if ig[y][x] == 1}
    if not ss: return SPREAD_ANCHORS[:n]
    cov, sel = set(), []
    for _ in range(n):
        b, bc = None, -1
        for a in ALL_A:
            c = len((AC[a] & ss) - cov)
            if c > bc: bc, b = c, a
        if not b or bc <= 0: break
        sel.append(b); cov |= (AC[b] & ss)
    return sel
def get_obs(ig, gt):
    obs1 = {(y,x): so(gt[y][x]) for y in range(40) for x in range(40)}
    obs2 = {(y,x): so(gt[y][x]) for y in range(40) for x in range(40)}
    ao = {}
    for a in sel_sett(ig, 5):
        for yx in AC[a]: ao[yx] = [obs1[yx]]
    for a in SPREAD_ANCHORS:
        for yx in AC[a]:
            if yx in ao: ao[yx].append(obs2[yx])
            else: ao[yx] = [obs2[yx]]
    return ao


def build_single_tensor(ig, cm, odm, sdm, ao, TEMP, N_HIST, shift_type="standard"):
    """Build one prediction tensor with given hyperparams."""
    rc = defaultdict(list)
    for yx, vals in ao.items():
        c = ig[yx[0]][yx[1]]
        if c not in STATIC_CODES:
            ct = ctx_fn(ig, yx[0], yx[1], odm, sdm)
            for v in vals: rc[ct].append(v)

    if shift_type == "standard":
        R = compute_shift(rc, cm)
        shifted = {ct: apply_shift(d, R) for ct, d in cm.items()}
    elif shift_type == "soft":
        R = compute_shift_soft(rc, cm)
        shifted = {ct: apply_shift(d, R) for ct, d in cm.items()}
    elif shift_type == "per_terrain":
        per_t_shifts = compute_shift_per_terrain(rc, cm)
        shifted = {}
        for ct, d in cm.items():
            code = ct[0]
            R = per_t_shifts.get(code, [1.0]*N)
            shifted[ct] = apply_shift(d, R)
    elif shift_type == "adaptive":
        R = compute_shift_adaptive(rc, cm)
        shifted = {ct: apply_shift(d, R) for ct, d in cm.items()}
    else:
        R = compute_shift(rc, cm)
        shifted = {ct: apply_shift(d, R) for ct, d in cm.items()}

    bl = {}
    for ct in set(list(shifted.keys()) + list(rc.keys())):
        h = shifted.get(ct, [1.0/N]*N)
        ol = rc.get(ct, [])
        nr = len(ol)
        if nr == 0: bl[ct] = h[:]; continue
        s = surprise(ol, h)
        nh = N_HIST_SURP if (s > SURP_THRESH and nr >= 5) else N_HIST
        rf = [0.0]*N
        for v in ol: rf[v] += 1.0/nr
        t = nr + nh
        bl[ct] = [(nr*rf[i] + nh*h[i])/t for i in range(N)]

    tensor = []
    for y in range(40):
        row = []
        for x in range(40):
            code = ig[y][x]
            if code in STATIC_CODES:
                pc = CODE_TO_CLASS[code]; d = [FS]*N; d[pc] = 1-5*FS
            else:
                ct = ctx_fn(ig, y, x, odm, sdm)
                prior = bl.get(ct, shifted.get(ct, [1.0/N]*N))[:]
                if (y, x) in ao:
                    vals = ao[(y, x)]; no = len(vals); oh = [0.0]*N
                    for v in vals: oh[v] += 1.0/no
                    d = [(1-ALPHA)*prior[i] + ALPHA*oh[i] for i in range(N)]
                else: d = prior[:]
                d = ts(d, TEMP)
                d = [max(v, FD) for v in d]
            t = sum(d); row.append([v/t for v in d])
        tensor.append(row)
    return tensor


def ensemble_tensors(tensors):
    """Average multiple tensors."""
    n = len(tensors)
    result = []
    for y in range(40):
        row = []
        for x in range(40):
            avg = [sum(t[y][x][c] for t in tensors)/n for c in range(N)]
            # Renormalize
            s = sum(avg)
            row.append([v/s for v in avg])
        result.append(row)
    return result


def main():
    history_dir = os.path.join(config.DATA_DIR, "round_history")
    files = sorted(f for f in os.listdir(history_dir) if f.endswith("_analysis.json"))
    rounds = defaultdict(list)
    for f in files: rounds[f.split("_seed")[0]].append(f)
    all_data = {}
    for f in files:
        with open(os.path.join(history_dir, f)) as fh: all_data[f] = json.load(fh)

    print("Precomputing distance maps...")
    odm_c = {}; sdm_c = {}
    for f in files:
        ig = all_data[f].get("initial_grid")
        if ig:
            odm_c[f] = bfs_dist(ig, 10)
            sdm_c[f] = bfs_dist(ig, 1)
    print(f"  Done: {len(odm_c)} maps.")

    round_ids = sorted(rounds.keys())
    strategies = ["PROD", "ENS_TEMP", "ENS_NHIST", "ENS_BOTH", "SHIFT_CLASS", "SHIFT_SOFT", "SHIFT_ADAPT", "N_HIST_40"]

    SEEDS = [42, 123, 456]

    print(f"\nEnsemble + Shift Test: {len(files)} files, {len(rounds)} rounds, {len(SEEDS)} seeds")
    print("=" * 120)
    hdr = f"  {'Round':<10}"
    for s in strategies: hdr += f" {s:>11}"
    print(hdr)
    print(f"  {'-'*10}" + f" {'-'*11}" * len(strategies))

    totals = {s: [] for s in strategies}

    for trid in round_ids:
        ca = defaultdict(list)
        for f in files:
            if f.split("_seed")[0] == trid: continue
            d = all_data[f]; gt = d.get("ground_truth"); ig = d.get("initial_grid")
            if not gt or not ig: continue
            odm = odm_c.get(f); sdm = sdm_c.get(f)
            for y in range(40):
                for x in range(40):
                    c = ig[y][x]
                    if c not in STATIC_CODES:
                        ca[ctx_fn(ig, y, x, odm, sdm)].append(gt[y][x])
        cm = {c: [sum(s[i] for s in ss)/len(ss) for i in range(N)] for c, ss in ca.items()}

        round_scores = {s: [] for s in strategies}

        for fname in rounds[trid]:
            d = all_data[fname]; gt = d.get("ground_truth"); ig = d.get("initial_grid")
            if not gt or not ig: continue
            odm = odm_c.get(fname); sdm = sdm_c.get(fname)

            for seed in SEEDS:
                random.seed(seed + all_data[fname].get("seed_index", 0))
                ao = get_obs(ig, gt)

                # PROD
                t = build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "standard")
                round_scores["PROD"].append(score_t(t, gt))

                # ENS_TEMP: ensemble 3 temperatures
                ts_list = [
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.05, 50, "standard"),
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "standard"),
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.15, 50, "standard"),
                ]
                round_scores["ENS_TEMP"].append(score_t(ensemble_tensors(ts_list), gt))

                # ENS_NHIST: ensemble 3 N_HIST
                ns_list = [
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 30, "standard"),
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "standard"),
                    build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 80, "standard"),
                ]
                round_scores["ENS_NHIST"].append(score_t(ensemble_tensors(ns_list), gt))

                # ENS_BOTH: ensemble 9 configs
                both_list = []
                for temp in [1.05, 1.10, 1.15]:
                    for nh in [30, 50, 80]:
                        both_list.append(build_single_tensor(ig, cm, odm, sdm, ao, temp, nh, "standard"))
                round_scores["ENS_BOTH"].append(score_t(ensemble_tensors(both_list), gt))

                # SHIFT variants
                t = build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "per_terrain")
                round_scores["SHIFT_CLASS"].append(score_t(t, gt))

                t = build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "soft")
                round_scores["SHIFT_SOFT"].append(score_t(t, gt))

                t = build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 50, "adaptive")
                round_scores["SHIFT_ADAPT"].append(score_t(t, gt))

                # N_HIST=40
                t = build_single_tensor(ig, cm, odm, sdm, ao, 1.10, 40, "standard")
                round_scores["N_HIST_40"].append(score_t(t, gt))

        avgs = {s: sum(round_scores[s])/max(1,len(round_scores[s])) for s in strategies}
        for s in strategies: totals[s].extend(round_scores[s])
        best = max(avgs, key=avgs.get)
        short = trid[:8]
        line = f"  {short:<10}"
        for s in strategies:
            m = "*" if s == best else " "
            line += f" {avgs[s]:10.1f}{m}"
        print(line)

    print(f"\n{'='*120}")
    prod_avg = sum(totals["PROD"]) / len(totals["PROD"])
    print(f"\n  {'Strategy':<14} {'Avg':>7} {'Delta':>7} {'Min':>7} {'Std':>7} {'Wins':>5}")
    print(f"  {'-'*14} {'-'*7} {'-'*7} {'-'*7} {'-'*7} {'-'*5}")
    for s in strategies:
        sc = totals[s]
        avg = sum(sc)/len(sc)
        mn = min(sc)
        std = (sum((v-avg)**2 for v in sc)/len(sc))**0.5
        wins = sum(1 for a, b in zip(totals["PROD"], sc) if b > a)
        delta = avg - prod_avg
        marker = " <<<" if delta > 0.3 else ""
        print(f"  {s:<14} {avg:7.2f} {delta:+7.2f} {mn:7.1f} {std:7.1f} {wins:>5}{marker}")

    print(f"\n  Done.")


if __name__ == "__main__":
    main()
